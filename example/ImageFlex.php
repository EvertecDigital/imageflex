...
class Imageflex {
    ...
    private static $watermarkPosition = 'middle center'; // Default watermark position
    ...
    public static function setCompression($compression) {
        self::$compression = intval($compression);
    }
    ...
    public static function setWatermarkOpacity($opacity) {
        self::$watermarkOpacity = min(max(intval($opacity), 0), 100);
    }
    ...
    public static function resize($imagePath, $width, $height = null) {
        ...
        // Determine scaling factor and new dimensions for cropping
        if ($originalAspectRatio > $newAspectRatio) {
            $scaleFactor = $height / $originalHeight;
            $srcW = intval($width / $scaleFactor);
            $srcH = $originalHeight;
            $srcX = intval(($originalWidth - $srcW) / 2);
            $srcY = 0;
        } else {
            $scaleFactor = $width / $originalWidth;
            $srcW = $originalWidth;
            $srcH = intval($height / $scaleFactor);
            $srcX = 0;
            $srcY = intval(($originalHeight - $srcH) / 2);
        }
